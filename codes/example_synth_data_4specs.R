...
for (isp in 1:NSPECS){
  ...
  pdistr = PDIST[isp]
  myptrue = PTRUE[[pdistr]]

  print(sprintf('synthetic specification distribution = %s', pdistr))
  print(sprintf('synthetic specification parameters = %s', myptrue))
  

# myptrue = list(a = 1.2, b = 0.12) # gam
# myptrue = list(xi = 0.1, sigma = 8) # gpd
...
# clrs <- color_scheme_get("brightblue")
    ...
    # outplot = file.path('../output/outplot')
    ggsave(file.path(outplot, sprintf('synth_examples_ss_20_50_4spec_%s.png', pdistr)), 
...
# ggsave(file.path(outplot, sprintf('wei_synth_pdfs_4spec_%s.png', pdistr)), plot=fig2, device = 'png',
#                        width = 6, height = 6)
