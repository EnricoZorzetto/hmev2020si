...
savename = file.path(outdata,
   sprintf('gof_synth_gener_%s.csv',numj)) # it includes the .csv - ugly..
...
rdf = expand.grid(test = tests, gof = gofs, numj = numj, ssize = myssize, spec = myspec, gen = mygen, 
  ...
  for (iig in 1:ngofs){
    ...
    rdf$value[rdf$gof == names(mygofs_ss)[iig] & rdf$test == 'ss'] = mygofs_ss[[iig]]
    rdf$value[rdf$gof == names(mygofs_cv)[iig] & rdf$test == 'cv'] = mygofs_cv[[iig]]
